...
solveProblem s = show . product .  take 2 . reverse . sort . map countInspects . rounds 20 $ monkeys
...
inspectAndThrow :: Int -> [Monkey] -> Int -> [Monkey]
...
startingItemsP :: P.GenParser Char st [Int]
startingItemsP = do
  P.spaces
  P.string "Starting items:"
  P.spaces
  P.sepBy intP (P.string ", ")

operationP :: P.GenParser Char st (Int -> Int)
operationP = do
  P.spaces
...
buildOperation :: Char -> Maybe Int -> Int -> Int
buildOperation '+' (Just num) = (+ num)
buildOperation '*' (Just num) = (* num)
buildOperation '+' Nothing = \x -> x+x
buildOperation '*' Nothing = \x -> x*x

testP :: P.GenParser Char st (Int -> Bool)
testP = do
  P.spaces
  P.string "Test: divisible by "
  num <- intP
  return (\x -> x `mod` num == 0)
