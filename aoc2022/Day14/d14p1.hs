...
solveProblem s = show . length . takeUntilFirstRepeat $ areasInTime
  where
    areasInTime = tail $ iterate dropASand areaWithSource
...
dropASand :: Area -> Area
dropASand area = dropSand areaWithSand (500,1)
  where areaWithSand = area//[((500,1),Sand)]

dropSand :: Area -> Coord -> Area
dropSand area (x,y)
  | (x,y+1) `isInBoundsOf`area && area!(x,y+1) == Air     = dropSand (area//[((x,y),Air), ((x,y+1),Sand)]) (x,y+1)
  | (x-1,y+1) `isInBoundsOf`area && area!(x-1,y+1) == Air = dropSand (area//[((x,y),Air), ((x-1,y+1),Sand)]) (x-1,y+1)
  | (x+1,y+1) `isInBoundsOf`area && area!(x+1,y+1) == Air = dropSand (area//[((x,y),Air), ((x+1,y+1),Sand)]) (x+1,y+1)
  | (x,y+1) `isOutOfBoundsOf`area || (x-1,y+1) `isOutOfBoundsOf`area || (x+1,y+1) `isOutOfBoundsOf`area = area//[((x,y),Air)]
  | otherwise = area
...
getBounds paths = ((minimum xs, y_min), (maximum xs, maximum ys))
  where
